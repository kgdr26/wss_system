            ...
            <div class="card">
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        <button class="btn btn-light-success" data-bs-toggle="modal" data-bs-target="#add_vendor">
                ...
                <div class="card-body py-4">
                    ...
                    <!--begin::Table-->
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="table_vendor">
                        <!--begin::Table head-->
                        <thead>
                            <!--begin::Table row-->
                        ...
                        <tbody class="text-gray-600 fw-semibold">
                            ...
                            @foreach ($arr as $key => $row)
                                <tr>
                                    ...
                                    <td class="fw-bold">{{$row->cp_name}}</td>
                                    <td>{{$row->name_vendor}} </td>
                                    <td>{{$row->npwp}}</td>
                                    <td>
                                        <p class="w-address-style">{{$row->address_vendor}}</p>
                                    </td>
                                    <td>{{$row->no_tlp}}</td>
...
<div class="modal fade" tabindex="-1" id="add_vendor">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="" value="" data-name="cp_name">
                </div>

                <div class="mb-3">
                    <label for="" class="form-label">NAME</label>
                    <input type="text" class="form-control" id="" value="" data-name="name_vendor">
                </div>

                <div class="mb-3">
                    <label for="" class="form-label">NPWP</label>
                    <input type="text" class="form-control" id="" value="" data-name="npwp">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="" id="" cols="30" rows="10" class="form-control" data-name="address_vendor"></textarea>
                </div>
            </div>

            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary" data-name="save_vendor">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).on("click", "[data-name='save_vendor']", function (e) {
        $('.preloader').show();
        var name_vendor = $('[data-name="name_vendor"]').val();
        var npwp = $('[data-name="npwp"]').val();
        var address_vendor = $('[data-name="address_vendor"]').val();
        var cp_name = $('[data-name="cp_name"]').val();
        var no_tlp = $('[data-name="no_tlp"]').val();

        $.ajax({
            type: "POST",
            url: "{{ route('addvendor') }}",
            data: {name_vendor:name_vendor,npwp:npwp,address_vendor:address_vendor,cp_name:cp_name,no_tlp:no_tlp},
            cache: false,
